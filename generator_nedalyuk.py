...
def User():
    ...
    for i in range(5):
        ...
        if coin > 0.5:
            ...
            listUser.append([i+100000, surnameF[indS], namesF[indN]])
...
def Transactions():
    ...
    for i in range(10):
        ...
        for a in range(len(fruits)):
            ...
            Seller = np.random.randint(0,len(IDSeller))
            if (quantity > 0):
                listTransactions.append([IDTrans, number, DATE, IDfruit[a], IDuser[USER], IDSeller[Seller], price[a]*quantity, quantity])
                IDTrans+=1
